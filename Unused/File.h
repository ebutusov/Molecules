...
public:
	...
	virtual void SetFilePath(string strNewName);

	virtual void Open(string strFileName, DWORD dwAccess, DWORD dwShareMode, 
		LPSECURITY_ATTRIBUTES lpSA, DWORD dwCreationDisposition, DWORD dwAttributes,
		HANDLE hTemplateFile) throw(CXFileException);

	static void Rename(string strOldName, string strNewName) throw(CXFileException);
	static void Remove(string strFileName) throw(CXFileException);
...
protected:
	string m_strFileName;	// stores the file name for the current file
};
